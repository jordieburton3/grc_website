...
const hoursToMilliseconds = (hours: number) => nearestHundredth(hours * 60 * 60 * 1000);
const minutesToMilliseconds = (minutes: number) => nearestHundredth(minutes * 60 * 1000);
const secondsToMilliseconds = (seconds: number) => nearestHundredth(seconds * 1000);
const millisecondsToHours = (milliseconds: number) => milliseconds / (60 * 60 * 1000);
const millisecondsToMinutes = (milliseconds: number) => (millisecondsToHours(milliseconds) % 1) * 60;
const millisecondsToSeconds = (milliseconds: number) => (millisecondsToMinutes(milliseconds) % 1) * 60;
...
export const convertToMilliseconds = ({ hours = 0, minutes = 0, seconds = 0 }): number => {
...
export const millisecondsToTime = (milliseconds: number): string => {
    ...
    return `${getHoursString(hours)}${getMinutesString(hours, minutes)}${getSecondsString(seconds)}`;
}