...
const images = [
	'https://dbukjj6eu5tsf.cloudfront.net/sidearm.sites/gatorzone.com/images/2016/2/13/Florida_Raz_Invite_136.jpg',
	'https://dbukjj6eu5tsf.cloudfront.net/sidearm.sites/gatorzone.com/images/2017/5/31/Taylor_Tubbs_DL.jpg'
];
...
export const ProfileCarousel: React.FunctionComponent = () => {
	...
	React.useEffect(() => {
		const timer = setInterval(() => {
			setImageToUse(
				imageToUseRef.current === images.length - 1
					? 0
					: imageToUseRef.current + 1
			);
		}, 5000);
	...
	React.useEffect(() => {
		window.addEventListener('resize', () => {
			...
			setMediaHeight({ ...mediaStylesRef, height: `${width * 0.66}px` });
		});
	}, []);
	return (
		<div className="profile-carousel__wrapper" style={mediaStyles}>
			{images.map((image, i) => (
				<img
					...
					src={image}
					key={image}
					className={`profile-carousel__image ${imageToUse === i
						? ''
						: AdditionalCarouselClasses.HIDDEN}`}
				/>
			))}
		</div>
	);
