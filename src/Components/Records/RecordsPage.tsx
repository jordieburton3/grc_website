...
const RecordsPage: React.FunctionComponent<IRecordsPageProps> = (props) => {
    ...
    return (
        <div className="records-page_wrapper">
            ...
            {getSortedRecordKeys(Object.keys(clubRecordsMap)).map(r => <ConnectedRecordSection records={clubRecordsMap[r]} key={r}/>)}
        </div>
    );
};
...
export const ConnectedRecordsPage: React.FunctionComponent = () => (
    <AppContext.Consumer>
        {context => (
            <RecordsPage clubRecordsMap={context.state.clubRecordsMap}/>
        )}
    </AppContext.Consumer>
);